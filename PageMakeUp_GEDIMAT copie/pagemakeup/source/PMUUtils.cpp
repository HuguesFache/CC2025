	...
	do
	{
		...
		if(doc != nil)
		{
			...
			{	
				...
				PermRefStruct permRef = PermRefsUtils::GetLastPermRef();
				if(permRef != kNullPermRef){
					selectedRefs.AppendNumber(permRef.ref);
					selectedParam1.AppendNumber(permRef.param1);
					selectedParam2.AppendNumber(permRef.param2);
					selectedParam3.AppendNumber(permRef.param3);
					selectedParam4.AppendNumber(permRef.param4);			
				}
			}
			...
			InterfacePtr<IDropDownListController> param1ListController(dialogView->FindWidget(kPMUParam1EditWidgetID), UseDefaultIID());
			InterfacePtr<IStringListControlData> param1ListControlData(param1ListController, UseDefaultIID());
			param1ListControlData->Clear(kFalse, kFalse);

			InterfacePtr<IDropDownListController> param2ListController(dialogView->FindWidget(kPMUParam2EditWidgetID), UseDefaultIID());
			InterfacePtr<IStringListControlData> param2ListControlData(param2ListController, UseDefaultIID());
			param2ListControlData->Clear(kFalse, kFalse);
			
			InterfacePtr<IDropDownListController> param3ListController(dialogView->FindWidget(kPMUParam3EditWidgetID), UseDefaultIID());
			InterfacePtr<IStringListControlData> param3ListControlData(param3ListController, UseDefaultIID());
			param3ListControlData->Clear(kFalse, kFalse);

			InterfacePtr<IDropDownListController> param4ListController(dialogView->FindWidget(kPMUParam4EditWidgetID), UseDefaultIID());
			InterfacePtr<IStringListControlData> param4ListControlData(param4ListController, UseDefaultIID());
			param4ListControlData->Clear(kFalse, kFalse);

			PMString item = kNullString;

			for(int32 i = 0; i <= REFERENCE_COUNT ; ++i){
				item.AppendNumber(i);
				item.SetTranslatable(kFalse);	

				param1ListControlData->AddString(item);
				param2ListControlData->AddString(item);
				param3ListControlData->AddString(item);
				param4ListControlData->AddString(item);

				item = kNullString;
			}

			param1ListController->Select(selectedParam1.GetAsNumber());	
			param2ListController->Select(selectedParam2.GetAsNumber());		
			param3ListController->Select(selectedParam3.GetAsNumber());
			param4ListController->Select(selectedParam4.GetAsNumber());
			...
			if(!blankEntry)
			{
				...
				InterfacePtr<IDropDownListController> param1ListController(dialogView->FindWidget(kPMUParam1EditWidgetID), UseDefaultIID());
				InterfacePtr<IStringListControlData> param1ListControlData(param1ListController, UseDefaultIID());
				int16 index1 = param1ListController->GetSelected();
				if(index1 != IDropDownListController::kNoSelection){
					PMString selectedRef1 = param1ListControlData->GetString(index1); 
					if(!blankEntry) 
						thePermRef.param1 = selectedRef1.GetAsNumber();
				}

				InterfacePtr<IDropDownListController> param2ListController(dialogView->FindWidget(kPMUParam2EditWidgetID), UseDefaultIID());
				InterfacePtr<IStringListControlData> param2ListControlData(param2ListController, UseDefaultIID());
				int16 index2 = param2ListController->GetSelected();
				if(index2 != IDropDownListController::kNoSelection){
					PMString selectedRef2 = param2ListControlData->GetString(index2); 
					if(!blankEntry) 
						thePermRef.param2 = selectedRef2.GetAsNumber();
				}

				InterfacePtr<IDropDownListController> param3ListController(dialogView->FindWidget(kPMUParam3EditWidgetID), UseDefaultIID());
				InterfacePtr<IStringListControlData> param3ListControlData(param3ListController, UseDefaultIID());
				int16 index3 = param3ListController->GetSelected();
				if(index3 != IDropDownListController::kNoSelection){
					PMString selectedRef3 = param3ListControlData->GetString(index3); 
					if(!blankEntry) 
						thePermRef.param3 = selectedRef3.GetAsNumber();
				}

				InterfacePtr<IDropDownListController> param4ListController(dialogView->FindWidget(kPMUParam4EditWidgetID), UseDefaultIID());
				InterfacePtr<IStringListControlData> param4ListControlData(param4ListController, UseDefaultIID());
				int16 index4 = param4ListController->GetSelected();
				if(index4 != IDropDownListController::kNoSelection){
					PMString selectedRef4 = param4ListControlData->GetString(index4); 
					if(!blankEntry) 
						thePermRef.param4 = selectedRef4.GetAsNumber();
				}			
...
void PMUUtils::AutoTagDocumentPermsRefsTCL() {
	...
	do{		 
		...
		// GD 15.09.2015 ++
		if(status == kSuccess) {
			status = PermRefsUtils::AutoTaggingDocumentPermRefs_GEDIMAT2015(doc);
		}
		// GD 15.09.2015 --
        
        // GD 02.09.2022 ++
        if(status == kSuccess) {
            status = PermRefsUtils::AutoTaggingDocumentPermRefs_GEDIMAT2022(doc);
        }
        // GD 02.09.2022 --
