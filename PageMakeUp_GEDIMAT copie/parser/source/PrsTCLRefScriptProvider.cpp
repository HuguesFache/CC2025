...
#endif
			data->InsertRequestData(propID, outputData);	
...
ErrorCode PrsTCLRefScriptProvider::AccessPermRefTagProperty(ScriptID propID, IScriptRequestData* data, IScript* script)
{
	...
	do 
	{
		InterfacePtr<ITextTarget> txtTarget (script, UseDefaultIID());
		...
		if (data->IsPropertyPut()) 
		{
			...
			if(PermRefsUtils::ApplyPermRefs(reference, txtModel, txtTarget->GetRange().Start(nil), txtTarget->GetRange().End()) != kSuccess) break;
		...
		{	
			...
			DataWrapper<AttributeBossList> localCharOverrides = charStrand->GetLocalOverrides(txtTarget->GetRange().Start(nil),nil);
